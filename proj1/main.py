...
# Подключение библиотеки для создания автоматической документации API
...
swagger = Swagger(app)
# Создаем основной Blueprint сайта
main = Blueprint("/", __name__, template_folder='templates',static_folder='static') # объявляем декоратор для метода http get
# Информация, которая будет выдаваться по URL/info/something
# Параметр в <> при вводе URL будет передан в переменную about функции info @main.route('/info/<about>/')
def info(about):
    ...
    responses:
     200:
      ...
      examples:
       version: '1.0'
    """
    all_info = {'all': 'main_author 1.0 2020',
                'version': '1.0',
                'author': 'main_author',
                'year': '2020'
      }
...
# Регистрируем основной Blueprint и Blueprint другой части сайта
